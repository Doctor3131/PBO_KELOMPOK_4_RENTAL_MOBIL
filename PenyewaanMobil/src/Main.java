...
//     private static ArrayList<Sopir> daftarSopir = new ArrayList<>();
//     private static ArrayList<Transaksi> daftarTransaksi = new ArrayList<>();
//     private static final int MAX_RENTAL_DAYS = 15;
//     private static final double LATE_FEE_PER_DAY = 100000; // Rp 100.000 per day
...
//             System.out.println("5. Tambah Penyewa Baru");
//             System.out.println("6. Buat Transaksi Baru");
//             System.out.println("7. Lihat Semua Transaksi");
//             System.out.println("8. Lihat Transaksi Aktif");
//             System.out.println("9. Selesaikan Transaksi");
//             System.out.println("10. Proses Keterlambatan Pengembalian");
...
//         // Add sample penyewa
//         daftarPenyewa.add(new Penyewa("3175020501990001", "Andi Saputra", "Jl. Kenanga No. 5", "089876543210"));
//         daftarPenyewa.add(new Penyewa("3175021002980002", "Dewi Lestari", "Jl. Mawar No. 7", "082345678901"));
//     }
...
//                 String status = isVehicleInUse(m.getID_Plat()) ? "Disewa" : "Tersedia";
...
//                 String status = isDriverInUse(s.getIdSopir()) ? "Disewa" : "Tersedia";
...
//         } else {
//             System.out.println("No. | ID | NIK              | Nama               | Telepon");
//             System.out.println("------------------------------------------------------------");
//             int i = 1;
//             for (Penyewa p : daftarPenyewa) {
//                 System.out.printf("%-3d | %-2d | %-16s | %-18s | %s%n", 
//                                  i++, p.getIdPenyewa(), p.getNik(), p.getName(), p.getPhoneNumber());
//             }
//         }
//         waitForEnter();
//     }
...
//         System.out.println("\nPilih Penyewa:");
//         for (int i = 0; i < daftarPenyewa.size(); i++) {
//             System.out.println((i + 1) + ". " + daftarPenyewa.get(i).getName() + " (NIK: " + daftarPenyewa.get(i).getNik() + ")");
//         }
//         System.out.print("Pilihan (1-" + daftarPenyewa.size() + "): ");
//         int penyewaChoice = getIntInput() - 1;
...
//             for (Mobil m : daftarMobil) {
//                 if (!isVehicleInUse(m.getID_Plat())) {
...
//             // Motor tidak menyediakan sopir
//             Transaksi transaksi = new Transaksi(selectedPenyewa, selectedMotor, durasiKendaraan);
//             processPembayaran(transaksi);
//         }
//     }
    
//     private static void processPembayaran(Transaksi transaksi) {
//         System.out.println("\nDetail Transaksi:");
//         transaksi.printInfo();
        
//         System.out.print("\nPilih metode pembayaran (1. Tunai, 2. Non-Tunai): ");
//         int paymentMethodChoice = getIntInput();
...
//             PembayaranNonTunai pembayaranNonTunai = new PembayaranNonTunai(
//                 transaksi.getPembayaran().getNominal(), resiBank);
...
//             transaksi.getPembayaran().showIdPembayaran();
//         }
        
//         daftarTransaksi.add(transaksi);
//         transaksi.getPenyewa().addTransaction(transaksi);
...
//             Transaksi t = activeTransaksi.get(i);
//             System.out.println((i + 1) + ". ID: " + t.getID_transaksi() + " - Penyewa: " + 
//                                t.getPenyewa().getName() + " - Kendaraan: " + getVehicleName(t));
//         }
...
//         // Calculate if there's any late return fee
//         LocalDate rentalDate = selectedTransaksi.getTanggal_transaksi();
//         LocalDate expectedReturnDate = rentalDate.plusDays(selectedTransaksi.getDurasi_kendaraan());
...
//             System.out.println("\nKendaraan terlambat dikembalikan sebanyak " + daysLate + " hari.");
//             System.out.println("Denda keterlambatan: Rp " + String.format("%,.2f", lateFee));
            
//             // Add late fee to total
//             double totalAmount = selectedTransaksi.getPembayaran().getNominal() + lateFee;
//             selectedTransaksi.getPembayaran().setNominal(totalAmount);
            
//             System.out.println("Total pembayaran (termasuk denda): Rp " + String.format("%,.2f", totalAmount));
//         }
...
//     private static void processLateReturn() {
//         clearScreen();
//         System.out.println("=== PROSES KETERLAMBATAN PENGEMBALIAN ===");
        
//         ArrayList<Transaksi> lateTransaksi = new ArrayList<>();
//         for (Transaksi t : daftarTransaksi) {
//             if (t.isActive()) {
//                 LocalDate rentalDate = t.getTanggal_transaksi();
//                 LocalDate expectedReturnDate = rentalDate.plusDays(t.getDurasi_kendaraan());
//                 LocalDate today = LocalDate.now();
...
//         if (lateTransaksi.isEmpty()) {
//             System.out.println("Tidak ada transaksi yang terlambat pengembalian.");
//             waitForEnter();
//             return;
//         }
        
//         System.out.println("Transaksi Terlambat:");
//         for (int i = 0; i < lateTransaksi.size(); i++) {
//             Transaksi t = lateTransaksi.get(i);
//             LocalDate rentalDate = t.getTanggal_transaksi();
//             LocalDate expectedReturnDate = rentalDate.plusDays(t.getDurasi_kendaraan());
//             LocalDate today = LocalDate.now();
...
//             System.out.println((i + 1) + ". ID: " + t.getID_transaksi() + 
//                               " - Penyewa: " + t.getPenyewa().getName() + 
//                               " - Kendaraan: " + getVehicleName(t) + 
//                               " - Terlambat: " + daysLate + " hari");
//         }
        
//         System.out.print("Pilih transaksi yang akan diproses (1-" + lateTransaksi.size() + "): ");
//         int choice = getIntInput() - 1;
        
//         if (choice < 0 || choice >= lateTransaksi.size()) {
//             System.out.println("Pilihan tidak valid!");
//             waitForEnter();
//             return;
//         }
        
//         Transaksi selectedTransaksi = lateTransaksi.get(choice);
...
//         System.out.println("Tanggal Sewa: " + rentalDate);
//         System.out.println("Tanggal Seharusnya Kembali: " + expectedReturnDate);
//         System.out.println("Tanggal Hari Ini: " + today);
//         System.out.println("Jumlah Hari Terlambat: " + daysLate);
//         System.out.println("Denda per Hari: Rp " + String.format("%,.2f", LATE_FEE_PER_DAY));
//         System.out.println("Total Denda: Rp " + String.format("%,.2f", lateFee));
        
//         // Add late fee to total
//         double totalAmount = selectedTransaksi.getPembayaran().getNominal() + lateFee;
        
//         System.out.println("\nTotal Pembayaran Awal: Rp " + String.format("%,.2f", selectedTransaksi.getPembayaran().getNominal()));
//         System.out.println("Total Pembayaran (termasuk denda): Rp " + String.format("%,.2f", totalAmount));
        
//         System.out.print("\nProses pembayaran denda? (y/n): ");
//         String processPayment = scanner.next().toLowerCase();
        
//         if (processPayment.equals("y")) {
//             selectedTransaksi.getPembayaran().setNominal(totalAmount);
            
//             System.out.print("\nPilih metode pembayaran denda (1. Tunai, 2. Non-Tunai): ");
//             int paymentMethodChoice = getIntInput();
